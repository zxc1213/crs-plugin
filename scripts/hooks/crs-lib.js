#!/usr/bin/env node
/**
 * CRS shared helpers for hooks (ClaudeCode protocol: JSON in via stdin, JSON out via stdout).
 * 同步自 crs-zcode v1.5.0 hooks/lib.mjs（ACTIVE 符号链接分支增加根目录边界校验）。
 */
...
export function readActiveRequirement(cwd) {
  ...
  try {
    const rawTarget = fs.readlinkSync(path.join(reqsDir, 'ACTIVE'));
    // 符号链接内容不可信：resolve 后必须仍落在 .requirements 边界内
    const reqPath = path.resolve(reqsDir, rawTarget);
    if (reqPath !== reqsDir && !reqPath.startsWith(reqsDir + path.sep)) {
      throw new Error(`ACTIVE target escapes .requirements: ${rawTarget}`);
    }
    const meta = parseMeta(fs.readFileSync(path.join(reqPath, 'meta.yaml'), 'utf-8'));
    if (meta.status) return { target: path.basename(reqPath), reqPath, status: meta.status };
  } catch (_err) {
