#!/usr/bin/env node
/**
 * CRS PostToolUse hook (ClaudeCode): execution log + rule-driven phase guard.
 * 同步自 crs-zcode v1.5.0 hooks/post-tool-use.mjs。
 *
 * For Edit/Write/Bash on a project with .requirements/:
 * - append a timestamped entry to <active-requirement>/execution.log
 ...
 *     | node scripts/hooks/crs-post-tool-use.js
 */

import fs from 'fs';
import path from 'path';
import { readActiveRequirement, readStdinJson, emitAdditionalContext, emitNothing } from './crs-lib.js';
...
try {
  const engine = await import('../requirement-manager/core/rules.js');
