#!/usr/bin/env node
/**
 * CRS SessionStart hook (ClaudeCode). 同步自 crs-zcode v1.5.0 hooks/session-start.mjs。
 *
 ...
 *     | node scripts/hooks/crs-session-start.js
 */
...
if (fs.existsSync(docsMapFile)) {
  try {
    const { checkDrift } = await import('../requirement-manager/project-sync/docs-map.js');
