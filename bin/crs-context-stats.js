#!/usr/bin/env node
/**
 * CRS 注入面体积统计（同步自 crs-zcode v1.5.0，FEAT-20260909-001-4ae874 任务 5）
 *
 * 度量插件注入 LLM 上下文的全部文本面：
 * - commands 下命令 .md 与 skills 下各 SKILL.md 的字节数（调用时整篇进入上下文）
...
// token 收敛目标：v1.4.0 基线 105,796B 的 75%；单命令 ≤5KB
// 注：claude 侧命令/技能数量多于 zcode（多平台入口保留），总量目标按本仓同步前实测基线等比放宽
...
function formatReport(data) {
  ...
  for (const [name, size] of Object.entries(data.commands)) {
    lines.push(`  ${name.padEnd(24)} ${String(size).padStart(6)} B`);
  }
  ...
  lines.push(
    `\n合计: commands ${t.commandsBytes} B + skills ${t.skillsBytes} B = ${t.sum} B` +
      `（zcode 收敛参考线 ≤${t.sumTarget} B：claude 多平台入口按本仓基线另行评估）`
  );
  lines.push(
