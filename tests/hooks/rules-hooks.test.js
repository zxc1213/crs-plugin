/**
 * rules-hooks.test.js - 三 hook 消费规则数据的端到端契约
 * 同步自 crs-zcode v1.5.0 tests/hooks/rules-hooks.test.js（hook 路径适配 scripts/hooks/crs-*.js），
 * 并并入旧 phase-gate.test.js 的语义用例（守卫改为 meta.yaml 扫描，不再依赖 ACTIVE 符号链接，
 * Windows 无符号链接权限也能跑——原 6 个 EPERM 失败用例的根因消除）。
 *
 * 关键契约：
 * - 无 rules.yaml 时注入文案与同步前行为逐字等价（行为不变重构）
...
import { readActiveRequirement } from '../../scripts/hooks/crs-lib.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const HOOKS_DIR = path.resolve(__dirname, '../../scripts/hooks');
const REPO_ROOT = path.resolve(__dirname, '../..');
const REQ_ID = 'FEAT-20260910-900-egg';
...
async function makeProject(dir, { status = 'planning', rulesYaml = null, logLines = 0, rawStatus = null } = {}) {
  const reqDir = path.join(dir, '.requirements', 'features', REQ_ID);
  await fs.mkdir(reqDir, { recursive: true });
  const statusLine = rawStatus !== null ? rawStatus : `status: ${status}`;
  await fs.writeFile(
  ...
  if (logLines > 0) {
    await fs.writeFile(
      ...
      Array.from({ length: logLines }, (_, i) => `[2026-09-10T00:00:0${i}] Tool: Edit`).join('\n'),
      'utf-8'
    );
  }
  return reqDir;
...
describe('hooks 消费规则数据（crs-*-*.js，ClaudeCode 事件协议）', function () {
  this.timeout(30000);
  let tmp;
  ...
  it('PostToolUse planning 阶段改外部文件 → 注入守卫文案（占位符展开）', async () => {
    await makeProject(tmp, { status: 'planning' });
    const target = path.join(tmp, 'src', 'app.js');
    const { out } = await runHook('crs-post-tool-use.js', {
  ...
  it('前缀混淆目录 .requirements-evil 不误判（视为外部，守卫命中）', async () => {
    await makeProject(tmp, { status: 'planning' });
  ...
  it('编辑 .requirements 内文件与 implementing 状态均零注入', async () => {
    await makeProject(tmp, { status: 'planning' });
    ...
    const { out: out2 } = await runHook('crs-post-tool-use.js', {
      hook_event_name: 'PostToolUse',
      tool_name: 'Write',
  ...
  it('Bash guard 按子串命中，大小写与空白归一', async () => {
  ...
  it('SessionStart 注入活跃需求消息 + inject 规则', async () => {
    await makeProject(tmp, {
      status: 'implementing',
      rulesYaml: ['rules:', '  - id: zh-commit', '    type: inject', '    message: 提交信息使用中文'].join('\n'),
    });
    const { out } = await runHook('crs-session-start.js', {
  ...
  it('无 rules.yaml 时 SessionStart 注入内置默认文案', async () => {
    await makeProject(tmp, { status: 'analyzed' });
    const { out } = await runHook('crs-session-start.js', { hook_event_name: 'SessionStart', cwd: tmp });
    const context = parseContext(out);
  ...
  it('inject 超预算按 priority 截断（整条保留/丢弃）', async () => {
  ...
  it('主消息 + inject 恰好等于预算保留，超 1 字符丢弃', async () => {
    await makeProject(tmp, { status: 'implementing' });
    const base = parseContext(
      (await runHook('crs-session-start.js', { hook_event_name: 'SessionStart', cwd: tmp })).out
    );
    ...
    await write(110);
    expect(parseContext((await runHook('crs-session-start.js', { hook_event_name: 'SessionStart', cwd: tmp })).out).length).to.equal(budget);

    await write(111); // +1 超预算 → 整条丢弃
    expect(parseContext((await runHook('crs-session-start.js', { hook_event_name: 'SessionStart', cwd: tmp })).out)).to.equal(base);
  });
  ...
  it('guard 多条命中按 priority 降序拼接，总量受 600 字符上限', async () => {
    const guardLine = (id, priority, tag) =>
      [
        ...
        `    message: ${tag}${'m'.repeat(132 - tag.length)}`,
      ].join('\n');
    await makeProject(tmp, {
      status: 'planning',
  ...
  it('Stop 汇总来自模板（含操作数与需求 ID）', async () => {
    ...
    const { out } = await runHook('crs-stop.js', { hook_event_name: 'Stop', cwd: tmp });
    const context = parseContext(out);
    expect(context).to.include(`active requirement ${REQ_ID}: 3 logged operations`);
  });

  it('hook 源码零内嵌面向用户文案', async () => {
    ...
    for (const name of ['crs-post-tool-use.js', 'crs-session-start.js', 'crs-stop.js']) {
  ...
  it('readActiveRequirement（meta.yaml 扫描，替代 ACTIVE 符号链接）：引号状态与缺状态字段', async () => {
    await makeProject(tmp, { rawStatus: 'status: "planning"' });
    const quoted = readActiveRequirement(tmp);
    expect(quoted).to.be.ok;
    expect(quoted.status).to.equal('planning');

    await makeProject(tmp, { rawStatus: '# no status field' });
    const noStatus = readActiveRequirement(tmp);
    expect(noStatus).to.be.null; // 无有效状态不视为活跃需求
  });
});
