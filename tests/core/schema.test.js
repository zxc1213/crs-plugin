...
describe('core/schema - 唯一口径源（同步自 crs-zcode v1.5.0）', () => {
  describe('常量定义', () => {
    it('状态生命周期完整且有序', () => {
    ...
    it('adjustment 前缀为 ADJU 且含 tech-debt→DEBT（文档口径纠错项）', () => {
      expect(TYPE_PREFIXES.adjustment).to.equal('ADJU');
      expect(TYPE_PREFIXES['tech-debt']).to.equal('DEBT');
    });
