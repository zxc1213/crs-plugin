...
describe('rules 引擎（FEAT-20260909-001-4ae874）', () => {
  ...
  it('无 rules.yaml 时返回纯内置默认，phase-guard 与 v1.4 行为一致', async () => {
  ...
  it('缺 id / 缺 message 的单条被剔除，其余生效', async () => {
    await writeRules(
      baseDir,
      [
        'rules:',
  ...
  it('type 非法 / enabled 非布尔剔除', async () => {
    await writeRules(
      baseDir,
      [
        'rules:',
  ...
  it('message 长度上限 inject 120 / guard 200（等于合法，超 1 剔除）', () => {
  ...
  it('inject_budget_chars 0 合法 / 负数与非数字回退 600 / 2001 收敛 2000', async () => {
  ...
  it('只写 version / rules 为空数组 → 内置规则保留', async () => {
  ...
  it('同 id 只覆盖 message 时 when 深保留（非整条替换）', async () => {
    await writeRules(
      baseDir,
  ...
  it('同进程缓存（二次调用同引用），resetRulesCache 后重读', async () => {
  ...
  it('priority 缺省为 0，同 id 重复时后者覆盖', () => {
  ...
  it('rules.yaml 是目录（不可读）时降级默认并告警一次', async () => {
